...
export async function handleSync(req,res){
  ...
  if(resource==='communities'){
    ...
    if(req.method==='GET'){const items=await collection.find({}).sort({updatedAt:-1}).limit(200).toArray();return res.status(200).json({communities:items.map(item=>{const joined=item.ownerId===userId||(item.members||[]).includes(userId),c={...item,_id:undefined,id:item.communityId,memberCount:(item.members||[]).length,joined},member=isAdmin||joined;if(!member)for(const key of privateCommunityFields)delete c[key];return c})})}
    ...
    if(req.method==='POST'&&req.body?.action==='leave'){const communityId=String(req.body.id||''),old=await collection.findOne({communityId});if(!old)return res.status(404).json({message:'모임을 찾지 못했습니다.'});if(old.ownerId===userId)return res.status(409).json({message:'모임 설립자는 탈퇴할 수 없습니다.'});await collection.updateOne({communityId},{$pull:{members:userId},$unset:{[`memberNames.${userId}`]:''},$set:{updatedAt:new Date()}});return res.status(200).json({ok:true})}

...
export async function handleSync(req,res){
  ...
  if(resource==='account-source'&&req.method==='GET'){const variants=ObjectId.isValid(userId)?[userId,new ObjectId(userId)]:[userId],accounts=await database.collection('account').find({userId:{$in:variants}},{projection:{providerId:1}}).toArray(),providers=accounts.map(account=>account.providerId);return res.status(200).json({email:session.user.email||'',source:providers.includes('naver')?'naver':providers.includes('credential')?'credential':'unknown'})}
  ...
  if(resource==='state'){
    ...
    if(req.method==='GET'){if(/^okeverybody7@naver\\.com$/i.test(String(session.user.email||'')))await collection.updateOne({userId,visitedCleanup20260822:{$ne:true}},{$set:{visited:[],visitedCleanup20260822:true,updatedAt:new Date()},$setOnInsert:{createdAt:new Date(),contributionCount:0}},{upsert:true});return res.status(200).json({state:await collection.findOne({userId})||{userId,visited:[],contributionCount:0}})};

...
export default async function handler(req,res){
  ...
  if(!objectIds.length)return res.status(200).json({names:{}});
  const users=await database.collection('user').find({_id:{$in:objectIds}},{projection:{name:1}}).toArray();
  return res.status(200).json({names:Object.fromEntries(users.map(user=>[String(user._id),user.name||'참여자']))});
}
